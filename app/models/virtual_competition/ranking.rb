class VirtualCompetition
  class Ranking
    ...
    attr_reader :competition, :page, :jump_kind

    def initialize(competition, scores, page: nil, jump_kind: nil)
    ...
    def all_scores = @all_scores ||= jump_kind ? filter_and_rank(@base_scores) : @base_scores
    ...
    def filter_and_rank(scores)
      ranked = scores.select { |score| score.track.kind == jump_kind }.sort_by(&:result)

...
class PerformanceCompetition::Category < ApplicationRecord
  self.table_name = :event_sections

  include EventOngoingValidation

  belongs_to :event, class_name: 'PerformanceCompetition', touch: true
  has_many :competitors, foreign_key: :section_id, inverse_of: :category, dependent: :restrict_with_error
  ...
  def lower_section
    @lower_section ||=
      event.categories.where('"order" > ?', order).limit(1).order(:order).first
  end

  def higher_section
    @higher_section ||=
      event.categories.where('"order" < ?', order).limit(1).order(order: :desc).first
  end
  ...
  def max_order_within_event
    event.categories.maximum(:order) || 0
  end

  def min_order_within_event
    event.categories.minimum(:order) || 0
  end
end

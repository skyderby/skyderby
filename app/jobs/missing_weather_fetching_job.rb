class MissingWeatherFetchingJob < ApplicationJob
  ...
  def perform(track_id)
    ...
    forecast_hour = gps_time.beginning_of_hour
    return if track.place.weather_data.for_time(forecast_hour).exists?

    places = Place.where(id: track.place)
    subregion = GfsForecast::Subregion.new(**places.to_subregion)

    Rails.logger.info <<~MSG
      Fetching weather for track ##{track_id} #{forecast_hour.iso8601}
      Place: #{track.place.name} (#{track.place.latitude}, #{track.place.longitude})
      Subregion: #{subregion} 
    MSG

...
RUN SECRET_KEY_BASE_DUMMY=1 ACTIVE_STORAGE_SERVICE=local HONEYBADGER_INSIGHTS_ENABLED=false bundle exec i18n export && \
    bundle exec bootsnap precompile app/ lib/ && \
    SECRET_KEY_BASE_DUMMY=1 ACTIVE_STORAGE_SERVICE=local HONEYBADGER_INSIGHTS_ENABLED=false bundle exec rails assets:precompile && \
    rm -rf node_modules

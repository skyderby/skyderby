WITH
  results_by_profile
    AS (SELECT DISTINCT ON (suits_kind, tracks.profile_id, wind_cancelled, virtual_competitions.id, DATE_PART('year', tracks.recorded_at))
          suits_kind,
          tracks.profile_id,
          tracks.suit_id,
          wind_cancelled,
          ...
          ROUND(
            (virtual_competition_results.result /
             MAX(virtual_competition_results.result)
             OVER (PARTITION BY virtual_competitions.id, wind_cancelled, DATE_PART('year', tracks.recorded_at)) *
             100)::numeric,
            1
          ) AS points,
  ...
  ranked_results_by_profile AS (SELECT
                                  results_by_profile.*,
                                    RANK()
                                    OVER (PARTITION BY competition_id, wind_cancelled, year ORDER BY results_by_profile.result DESC) AS rank
                                FROM
                                  results_by_profile)
...
SELECT
  ...
  year,
    RANK()
    OVER (PARTITION BY group_id, suits_kind, wind_cancelled, year ORDER BY SUM(ranked_results_by_profile.points) DESC) AS rank,
  JSON_OBJECT_AGG(

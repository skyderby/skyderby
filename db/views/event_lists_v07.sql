...
FROM (
  ...
  LEFT JOIN (
    ...
    GROUP BY
      categories.event_id, categories.name
  ) AS competitors_count
  ON events.id = competitors_count.event_id
  LEFT JOIN (
    SELECT
  ...
  SELECT
    ...
    series.created_at
  FROM
    competition_series AS series
    LEFT JOIN competition_series_included_competitions AS included_competitions
      ON series.id = included_competitions.competition_series_id
    ...
    LEFT JOIN (
      SELECT
        ...
        count(competitors.id) AS count
      FROM event_competitors AS competitors
      INNER JOIN competition_series_included_competitions AS included_competitions

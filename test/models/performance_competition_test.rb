...
class EventTest < ActiveSupport::TestCase
  test 'initial state' do
    ...
    travel_to date
    user = create :user
    event = PerformanceCompetition.create!(name: 'Test event', responsible: user, starts_at: Time.zone.today)
  ...
  test 'permanently_delete removes event and associated records' do
    ...
    3.times do
      create(
        :event_result,
        round: round,
        competitor: create(:event_competitor, event:, category:),
    ...
    assert_empty PerformanceCompetition::Category.where(event:)
    assert_empty PerformanceCompetition::Competitor.where(event:)
    assert_empty PerformanceCompetition::Round.where(event:)
